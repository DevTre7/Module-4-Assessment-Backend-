...
const getGoodGreetings = () => {
  axios.get("http://localhost:4000/api/goodGreetings/").then((res) => {
    const data = res.data;
    alert(data);
  });
};
...
const handleInput = () => {
  ...
  axios
    .post("http://localhost:4000/api/addUserInput/", {
      value: inputSpace.value,
    })
    .then((res) => {
      const data = res.data;
      alert(data);
...
const getAllUserInputs = () => {
  axios.get("http://localhost:4000/api/getAllUsers/").then((res) => {
    const data = res.data;
    alert(data);
  });
};
getAllUserInputsButton.addEventListener("click", getAllUserInputs);
